...
CCObject* NDKHelper::GetCCObjectFromData(void *obj)
{
        return NULL;
}
...
void NDKHelper::HandleMessage(json_t *methodName, void* data)
{
    CCLOG("Da vao toi handle message");
    if (methodName == NULL)
        return;
    ...
    for (unsigned int i = 0; i < NDKHelper::selectorList.size(); ++i)
    {
        if (NDKHelper::selectorList[i].getName().compare(methodNameStr) == 0)
        {
            ...
            CCFiniteTimeAction* action = CCSequence::create(CCCallFuncND::create(target, sel, (void*)data), NULL);
...
extern "C"
{
    ...
    void Java_com_easyndk_classes_AndroidNDKHelper_CPPNativeCallWithDataHandler(JNIEnv* env, jobject thiz, jstring json, jbyteArray  inArray)
    {
        string jsonString = JniHelper::jstring2string(json);
        const char *jsonCharArray = jsonString.c_str();
        ...
        jint len = env->GetArrayLength(inArray);
        char* data = new char[len];
        env->GetByteArrayRegion (inArray, (jint)0, (jint)len, (jbyte*)data);
...
//        data = newdata;
        ...
        env->SetByteArrayRegion (result, (jint)0, (jint)len , (jbyte*)data);
        ...
        //NDKHelper::HandleMessage(jsonMethodName, jsonMethodParams);
        NDKHelper::HandleMessage(jsonMethodName, (void*)data);
        CCLOG("Da vao toi CPPNativeCallWithDataHandler");
    ...
    void SendMessageWithData(string methodName, void* data)
    {
        if (0 == strcmp(methodName.c_str(), ""))
            return;
        json_t *toBeSentJson = json_object();
                                           ...
                                           "(Ljava/lang/String;)V"))
		{
            ...
            jbyteArray result = t.env->NewByteArray(len );
            t.env->SetByteArrayRegion (result, (jint)0, (jint)len , (jbyte*)dataNew);
        ...
        IOSNDKHelperImpl::RecieveData(jsonMessageName, data);
