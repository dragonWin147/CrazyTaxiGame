...
public class AndroidNDKHelper
{
	...
	private static native void CPPNativeCallWithDataHandler(String json, byte[] data);
	...
	private final static int __MSG_FROM_CPP_DATA__ = 6;
		...
		NDKHelperHandler = new Handler()
		{
        	public void handleMessage(Message msg)
        	{
        		switch(msg.what)
        		{
	        		...
	        		break;
	        		case __MSG_FROM_CPP_DATA__:
	        			try
	        			{
	        				NDKMessage message = (NDKMessage)msg.obj;
	        				message.methodToCall.invoke(AndroidNDKHelper.callHandler, message.methodParams, message.data);
	        			}
	        			catch (IllegalArgumentException e)
	        			{
	        				// TODO Auto-generated catch block
	...
	public static void SendMessageWithData(String methodToCall, byte[] data)
	{
		JSONObject obj = new JSONObject();
		try
		{
			...
			CPPNativeCallWithDataHandler(obj.toString(), data);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static void RecieveCppMessageWithData(String json, byte[] data)
	{
		if (json != null)
		{
			try
			{
				...
				if (obj.has(__CALLED_METHOD__))
				{
					...
					try
					{
						...
						NDKMessage message = new NDKMessage();
						message.methodToCall = m;
						message.methodParams = null;
						message.data = data;
						
						Message msg = new Message();
				    	msg.what = __MSG_FROM_CPP_DATA__;
				    	msg.obj = message;
